...
public class ItemModelBed extends ItemModelStandard {
	public static IconCoordinate[] seatIcons = new IconCoordinate[DyeColor.COLOR_AMOUNT];

	public ItemModelBed(Item item) {
		super(item, MOD_ID);
	}
	...
	static {
		for(DyeColor c : DyeColor.itemOrderedColors()) {
			seatIcons[c.itemMeta] = TextureRegistry.getTexture(c == DyeColor.RED ? "minecraft:item/bed" : MOD_ID + ":item/bed_" + c.colorID);
		}
